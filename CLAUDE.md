...
### Routing
Based on **GoRouter** (^12.1.3):
- Route definitions in `lib/core/router/app_router.dart`
- Routes use constants: `AppRouter.splash`, `AppRouter.login`, `AppRouter.home`
- Navigation via `AppNavigator` helper class:
  - `AppNavigator.toLogin()` / `toHome()` for specific routes
  - `AppNavigator.logout()` clears storage and redirects
- Auth-aware redirects: checks `StorageManager.isLoggedIn()` and auto-redirects unauthorized users
...
3. Define state class with `copyWith()` method
4. Create ViewModel extending `StateNotifier<YourState>`
5. Create Provider: `final yourProvider = StateNotifierProvider<YourViewModel, YourState>(...)`
6. Add route to `lib/core/router/app_router.dart`
7. Optionally add navigation helper to `AppNavigator` class
...
- Login: Call API → `StorageManager.saveToken(token)` → `StorageManager.saveUserInfo(info)` → `AppNavigator.toHome()`
- Logout: `AppNavigator.logout()` (clears token/info and redirects)
...
**Code gen**: auto_route, build_runner, hive_generator, json_serializable
